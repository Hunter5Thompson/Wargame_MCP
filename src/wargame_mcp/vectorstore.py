...
from .instrumentation import get_correlation_id, logger, track_latency
...
def normalize_metadata(metadata: dict | None) -> dict:
    ...
    if isinstance(tags_value, str):
        normalized["tags"] = [tag for tag in tags_value.split(",") if tag]
    return normalized
...
def upsert_chunks(chunks: Iterable[DocumentChunk], embeddings: list[list[float]]) -> None:
    ...
    metadatas: list[dict] = []
    documents: list[str] = []
    for chunk, _vector in zip(chunks, embeddings):
        ids.append(chunk.id)
        metadatas.append(chunk.chroma_metadata())
        documents.append(chunk.text)
    with track_latency("vectorstore.upsert", records=len(ids)):
        collection.upsert(
...
def delete_document(document_id: str) -> None:
    collection = get_collection()
    with track_latency("vectorstore.delete", document_id=document_id):
        collection.delete(where={"document_id": document_id})
    logger.info("vectorstore.delete", document_id=document_id, correlation_id=get_correlation_id())
...
class _InMemoryCollection:  # pragma: no cover - fallback
    ...
    def query(self, query_embeddings: Embeddings, n_results: int, where: dict | None = None) -> dict:
        ...
        return {
            ...
            "metadatas": [[normalize_metadata(record["metadata"]) for _, _, record in top]],
    ...
    def get(self, where: dict | None = None, include: list[str] | None = None) -> dict:
        ...
        if "metadatas" in include:
            response["metadatas"] = [normalize_metadata(record["metadata"]) for _, record in matches]
        return response
